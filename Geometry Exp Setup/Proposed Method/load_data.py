...
def read_book_data(book_content_file):
    df = pd.read_csv(book_content_file, dtype = "string", encoding = "utf-8")
    all_data = {}
    for i in range(df.shape[0]):
...
# def clean_section_list(section_list):
#     final_section_list =

def get_section_data(book_data):
    section_list = [str(book_data[i]["section"]) for i in range(len(book_data))]
    section_data = {}
...
def get_section_content(section, section_data, book_data):
    ...
    req_sections = section_data[str(section)].split("|")
    content = ""
