...
def get_final_index(df):
	...
	concept_sections = df[["book1_fo"]].values
	final_sections = []
	for concept_data in concept_sections:
		if concept_data != '':
			final_sections.append(concept_data[0])
		else:
			final_sections.append('8.7')
		# sections = [concept_data[j] for j in range(1) if concept_data[j] == concept_data[j]]
...
def get_concept_ranking(concept_index, book_file):
    df_book = pd.read_csv(book_file, dtype = "string")["section"]
