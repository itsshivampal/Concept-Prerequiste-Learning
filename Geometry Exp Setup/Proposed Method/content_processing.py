...
def read_book_data(file_name):
    df = pd.read_csv(file_name, encoding = "utf-8", dtype = "string")
    all_data = {}
...
def get_section_data(book_data):
    ...
    for i in range(len(section_list)):
        ...
        for j in range(i+1, len(section_list)):
            x1 = len(str(section_list[i]).split("."))
            x2 = len(str(section_list[j]).split("."))
            if x2 > x1:
...
def get_concept_content(df_concept_data, book_data_file):
    ...
    df_concept_data.fillna('', inplace=True)
    concept_sections = df_concept_data[["book1_fs"]].values
    final_sections = []
    for concept_data in concept_sections:
        if concept_data != '':
            final_sections.append(concept_data[0])
        else:
